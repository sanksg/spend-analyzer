...
const SQLIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 text-muted-foreground hover:text-primary">
        ...
        <div className="bg-card rounded-xl shadow-lg border border-border flex flex-col flex-1 overflow-hidden">
            <div className="flex-1 p-6 overflow-y-auto bg-muted/50">
                {!history ? (
                    <div className="flex flex-col items-center justify-center h-full text-muted-foreground space-y-6">
                        <div className="bg-card p-6 rounded-full shadow-sm border border-border/60">
                            <svg className="w-12 h-12 text-primary/60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>
                        </div>
                        <h3 className="text-xl font-semibold text-muted-foreground">What would you like to know?</h3>
                    ...
                    <div className="space-y-8 max-w-3xl mx-auto">
                        ...
                        <div className="flex justify-end">
                            <div className="bg-primary text-primary-foreground px-5 py-3 rounded-2xl rounded-tr-none shadow-md max-w-[80%]">
                        ...
                        <div className="flex justify-start w-full">
                            <div className="bg-card border border-border px-6 py-5 rounded-2xl rounded-tl-none shadow-md w-full space-y-4">

                                {/* 1. Prioritize Single Value Answer (e.g. Total Spend) */}
                                {isSingleResult ? (
                                    <div className="py-2">
                                        <p className="text-sm text-muted-foreground uppercase tracking-wide mb-1">Answer</p>
                                    ...
                                    <div className="prose prose-slate max-w-none">
                                        <p className="text-foreground text-lg leading-relaxed whitespace-pre-wrap">{history.answer}</p>
                                    </div>
                                )}
                                ...
                                {hasData && !isSingleResult && (
                                    <div className="mt-4 border rounded-lg overflow-hidden">
                                        <div className="bg-muted/50 px-4 py-2 text-xs font-semibold text-muted-foreground uppercase border-b border-border">
                                            Result Data
                                        </div>
                                                ...
                                                <thead className="bg-muted/50">
                                                    <tr>
                                                        {Object.keys(history.raw_data[0]).map(k => (
                                                            <th key={k} className="px-3 py-2 text-left font-medium text-muted-foreground capitalize">{k.replace(/_/g, " ")}</th>
                                                        ))}
                                                    </tr>
                                                </thead>
                                ...
                                {/* 4. Empty State */}
                                {!hasData && (
                                    <div className="bg-muted/50 border border-border/60 rounded-lg p-4 text-center">
                                        <p className="text-muted-foreground">I couldn't find any relevant data for that query.</p>
                                    </div>
                                )}
                                ...
                                {showSQL && (
                                    <div className="mt-3 animate-in fade-in duration-200">
                                        <p className="text-xs text-muted-foreground uppercase font-semibold mb-1">Generated SQL</p>
                                        <div className="bg-foreground rounded-lg p-3 overflow-x-auto">
                                            <code className="text-xs font-mono text-success block whitespace-pre">
            ...
            <div className="p-6 border-t border-border/60 bg-card">
                <form onSubmit={handleSubmit} className="max-w-3xl mx-auto flex gap-3">
                    <input
                        type="text"
                        className="flex-1 border border-input bg-background rounded-xl px-5 py-4 text-lg focus:ring-2 focus:ring-ring focus:border-input outline-none transition shadow-sm"
                    ...
                    <button
                        ...
                        className="bg-primary hover:bg-primary/90 text-primary-foreground px-8 rounded-xl font-medium transition disabled:opacity-50 flex items-center shadow-sm"
                    >
...
function SuggestionCard({ text, onClick }: { text: string, onClick: () => void }) {
    return (
        <button
            onClick={onClick}
            className="text-sm text-muted-foreground bg-muted/50 hover:bg-accent hover:text-accent-foreground border border-border rounded-lg px-4 py-3 transition text-left"
        >
            {text}
        </button>
    )
